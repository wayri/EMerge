...
def _ne2(coeff, coords, i, j, k):
    ...
    ) * (ai - aj + bi * xs - bj * xs + ci * ys - cj * ys + di * zs - dj * zs)
    by = -(
...
def _ne2_curl_tri(coeff, coords, i, j, k):
    ...
    ys = coords[1, :]
    out = (
        -(bi - bj) * (ci * (aj + bj * xs + cj * ys) - cj * (ai + bi * xs + ci * ys))
        + (ci - cj) * (bi * (aj + bj * xs + cj * ys) - bj * (ai + bi * xs + ci * ys))
        + 2 * (bi * cj - bj * ci) * (ai - aj + bi * xs - bj * xs + ci * ys - cj * ys)
    )
    return out.astype(np.complex128)
...
def _ne2_div_tri(coeff, coords, i, j, k):
    ...
    ys = coords[1, :]
    out = -(bi - bj) * (
        bi * (aj + bj * xs + cj * ys) - bj * (ai + bi * xs + ci * ys)
    ) - (ci - cj) * (ci * (aj + bj * xs + cj * ys) - cj * (ai + bi * xs + ci * ys))
    return out.astype(np.complex128)
